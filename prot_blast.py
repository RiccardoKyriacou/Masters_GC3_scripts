...
parse.add_argument("-i", "--input",type=str, help="name of input tsv file to get gene IDs",required=True)
...
for sp, genes in sp_genes_dict.items():
    ...
    with open(args.output + sp + '_query_prot.fasta', 'w') as outF, open(cds_fasta) as f:
            ...
            for prot_query, hits_info in prot_hits.items():
                with open(args.output + sp + '_' + prot_query.split('.')[0] + '_blast_prot.fasta', 'w') as outF:
                    ...
                    for hit_info in hits_info:
                        ...
                        outF.write('>' + sp_name + "_" + transcript_id + '\n' + hit_ID_seq + '\n')
    else:
        continue 

cds_dict = {} #dictionary of cds_IDS and cds sequences from --cds
...
sp_dict_cds = {} #Dictionary of species names as the gene id for that species
        ...
        for gene_query, transcript_header in cds_hits.items():
            with open(args.output + sp_name + '_' + gene_query.split('.')[0] + '_blast_cds.fasta', 'w') as OutF:
                ...
                for hit in transcript_header:
                    ...
                    OutF.write(">" + hit_sp_name + "_" + transcript_id + "\n" + hit_cds_seq +  "\n")
...
with open("combined_outlier_genes.tsv", "r") as f, open(args.output + "all_gene_hits.tsv", "w") as OutF:
    ...
    for line in f:
        ...
        if len(gene_protein) > 0:
            ...
            OutF.write(gene_id + "\t" + gene_protein_hits + "\t" + E_value + "\t" + Identities + "\n")

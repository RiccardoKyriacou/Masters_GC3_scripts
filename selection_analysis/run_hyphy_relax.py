...
#Fucntion to run HyPhy in parallel
def parallel_relax_run(output_dir, n_jobs):
    job_dict = {} 
    ...
    Parallel(n_jobs=n_jobs)(delayed(run_hyphy_relax)(alignment, tree) for alignment, tree in job_dict.items())   

def main():
    #Move files into a new dir
    ...
    number_of_jobs = int(len(BUSCO_lst))
    parallel_relax_run(args.out, number_of_jobs)
